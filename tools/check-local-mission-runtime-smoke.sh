...
OURBOX_AIRGAP_PLATFORM_CREATED=2026-03-23T00:00:00Z
OURBOX_PLATFORM_CONTRACT_DIGEST=${PLATFORM_DIGEST}
...
run_loader() {
  ...
  env \
    ...
    bash -lc '
      ...
      printf "OURBOX_AIRGAP_PLATFORM_REF=%s\n" "${OURBOX_AIRGAP_PLATFORM_REF}"
      printf "OURBOX_AIRGAP_PLATFORM_DIGEST=%s\n" "${OURBOX_AIRGAP_PLATFORM_DIGEST}"
      printf "OURBOX_AIRGAP_PLATFORM_ARCH=%s\n" "${OURBOX_AIRGAP_PLATFORM_ARCH}"
      printf "OURBOX_AIRGAP_PLATFORM_PROFILE=%s\n" "${OURBOX_AIRGAP_PLATFORM_PROFILE}"
...
grep -Fq "OURBOX_AIRGAP_PLATFORM_REF=ghcr.io/example/airgap-platform@${AIRGAP_DIGEST}" <<<"${loader_output}" \
  || die "matchbox runtime did not load the resolved application bundle ref"
grep -Fq "OURBOX_AIRGAP_PLATFORM_DIGEST=${AIRGAP_DIGEST}" <<<"${loader_output}" \
  || die "matchbox runtime did not load the resolved application bundle digest"
grep -Fq "OURBOX_AIRGAP_PLATFORM_ARCH=arm64" <<<"${loader_output}" \
  || die "matchbox runtime did not load the application bundle arch"
grep -Fq "OURBOX_AIRGAP_PLATFORM_PROFILE=demo-apps" <<<"${loader_output}" \
